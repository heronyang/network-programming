...
void parse_param(const char *str) {
    ...
    for( i=0 ; result[i]!=NULL ; i++) {
        ...
        if( strlen(ind) == 2 && (sscanf(ind, "%c%d", &ele, &num) != 2) )            perror("scanf");
        if( strlen(ind) == 3 && (sscanf(ind, "%c%c%d", &ele, &ele2, &num) != 3) )   perror("scanf");
        num --;
        ...
        } else if(ele == 's' && ele2 == 'h') {
            req[num].sh = malloc(REQUEST_CONTENT_LENGTH);
            strcpy(req[num].sh, val);
        } else if(ele == 's' && ele2 == 'p') {
            req[num].sp = malloc(REQUEST_CONTENT_LENGTH);
            strcpy(req[num].sp, val);
        }
